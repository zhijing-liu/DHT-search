...
### 同步策略
- 启动时按 `last_rowid` **增量补录**已有新增行（**仅在后台执行**，不阻塞服务启动与请求）；运行期不再有「每小时自动增量同步」，统一由**定时全量重建**接管（见 `REINDEX_CRON`）；
- 源库中对已有行的 UPDATE / DELETE 不会自动反映，需手动触发 `POST /api/reindex` 或等待下次定时全量重建（重建 JOIN 会自动丢弃源中已删除的残留行）；tokenizer 变更等结构性变更同样交由全量重建处理。
...
- **在线重建与定时维护**：全量重建在独立 worker 线程（Node）或子进程（Bun）执行，**主进程零阻塞，重建期间页面与检索全程可用**；启动同步在后台执行、服务秒级可用，运行期周期维护由 `REINDEX_CRON` 定时全量重建承担。
...
| `REINDEX_CRON` | `''` | 定时全量重建的 cron 表达式（标准 5 字段，如 `'0 4 * * *'` = 每天 04:00）；空字符串（默认）关闭，启用后 cron 成为唯一周期索引维护（取代旧的每小时自动增量同步） |
...
- **增量同步**：启动时会按 `last_rowid` 补录一次源库新增行；运行期不再自动增量同步。周期维护改为 `REINDEX_CRON` 定时全量重建（默认关闭），启用后到点在后台重建一次。
...
| 定时重建索引的时机 | `REINDEX_CRON` | cron 表达式（如 `'0 4 * * *'` 每天 04:00）；留空 `''` 表示关闭定时重建，仅靠启动同步 + 手动操作 |
...
- **数据库有新数据了**：程序启动时会自动补录一次新增行；若要周期性自动刷新，配置 `REINDEX_CRON`（如 `'0 4 * * *'`）让每天低峰定时全量重建，也可以在网页「设置」面板里手动点「同步」立即补录；
