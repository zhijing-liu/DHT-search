...
### 同步策略
- 启动时按 `last_rowid` **增量补录**已有新增行（**仅在后台执行**，不阻塞服务启动与请求）；运行期不再有「每小时自动增量同步」，统一由**每日定时增量同步**接管（见 `SYNC_CRON`；全量重建仅手动触发）；
- 源库中对已有行的 UPDATE / DELETE 不会自动反映，需手动触发 `POST /api/reindex` 全量重建（重建 JOIN 会自动丢弃源中已删除的残留行）；tokenizer 变更等结构性变更同样交由手动全量重建处理。
...
前端源码在 `web/src/`（Alpine.js 声明式模板 + Tailwind CSS，Vite 打包），
页面渲染与事件绑定全部由 `index.html` 上的 Alpine 指令驱动，`web/src/` 只提供状态、动作与纯函数。
...
```
DHT-search/
├── index.js                   # Express 入口：HTTP API + 静态资源 + 搜索缓存/定时重建/运行状态 SSE
...
│   ├── package.json
│   └── src/
│       ├── main.js            # 入口：注册 Alpine 组件与 store 后启动
│       ├── app.js             # 页面主组件：状态 + 动作（搜索/排序/筛选/分页/热词/黑名单/设置/运行状态/URL 同步）
│       ├── card.js            # 结果卡片组件：标题高亮 / 文件预览 / 详情弹窗与文件树
│       ├── api.js             # 后端接口层（统一取 JSON 与错误信息）
│       ├── toast.js           # 全局通知的 Alpine store
│       ├── icons.js           # 模板中动态渲染的图标字面量
│       ├── file-tree.js       # 由扁平 [{path,size}] 构建可折叠目录树
│       ├── util.js            # 共享纯函数（格式化、高亮、联想匹配、复制、RPC 推送等）
...
- **整体视图切换（搜索 / 资源库）**：顶栏分段按钮切换两套视图。资源库走 `/api/latest`，按 `id` 倒序（表倒序）列出最新入库的资源——**只有分页**，不提供关键词、排序与过滤，结果列表与服务端分页与搜索视图复用同一套实现。
- **URL 状态同步**：视图类型记在 **hash**（`#latest` 为资源库，无 hash 即搜索），视图内的关键词 / 排序 / 体积筛选 / 翻页 / 每页条数记在 **query string**；两者都会写回地址栏**并记入浏览器历史**，后退 / 前进可逐步还原任意视图。链接可直接分享或刷新保持（原地重取如「刷新按钮 / 同步重建后的重搜」不产生多余历史记录）。
...
- **在线重建与定时维护**：全量重建在独立 worker 线程（Node）或子进程（Bun）执行，**主进程零阻塞，重建期间页面与检索全程可用**；启动同步在后台执行、服务秒级可用，运行期周期维护由 `SYNC_CRON` 每日定时增量同步承担（全量重建仅手动触发）。
- **运行状态监测**：设置面板经 SSE（`/api/stats/stream`）每 3 秒推送缓存命中、堆内存、搜索进程数、已索引总数、下次同步倒计时（由 `SYNC_CRON` 推算出的下一次触发点，非「上次 + 固定间隔」）与重建/同步进度。
...
2. **运行时**（二选一）：
   - **Node**：需 `better-sqlite3`（C++ 原生模块，`npm install` 会自动尝试预编译/本地编译）。要求 Node ≥ 20（支持顶层 await），**推荐 22 / 24 LTS**（Node 20 已于 2026-04 EOL，CI 不再测试）。
...
| GET | `/api/latest` | 最新入库列表（不经 FTS，按入库顺序从新到旧；前端「资源库」视图使用，见下） |
...
- **定时增量同步**：启动时会按 `last_rowid` 补录一次源库新增行；运行期周期维护由 `SYNC_CRON`（默认关闭）到点增量补录新增行，秒级完成；全量重建仅手动触发（`POST /api/reindex`）。
...
| 定时增量同步的时机 | `SYNC_CRON` | cron 表达式（如每天 03:00 填 `'0 3 * * *'`）；默认关闭，留空 `''` |
...
- **数据库有新数据了**：程序启动时会自动补录一次新增行；运行期可配置 `SYNC_CRON`（如每天 03:00）定时增量补录，也可以在网页「设置」面板里手动点「同步」立即补录；源库已有行的修改/删除则需手动点「重建索引」全量刷新；
