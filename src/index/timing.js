...
export function createIndexTimer() {
  const startedAt = now();
  let measured = 0; // 已被归类到具体阶段的毫秒数（用于派生 js = total - measured）
  ...
  return {
    ...
    snapshot() {
      ...
      out.js = Math.max(0, Math.round(now() - startedAt - measured));
      out.totalMs = Math.round(now() - startedAt);
      return out;
    },
    /** 单行摘要（日志用） */
    summary() {
