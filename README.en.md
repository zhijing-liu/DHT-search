...
| `FILES_COMPRESS` | `true` | zlib-compress `files` in the cold DB; only the detail endpoint pays to inflate |
...
The cold DB holds `magnets_files` / `magnets_preview`. **Why split**: `files` + `preview` were most of
the old copy table's volume, and every slow query has the shape "collect N matching rowids → look each
up by primary key", whose cost is governed by the copy table's page count — a wide table means random
disk IO, a narrow one stays cached.
